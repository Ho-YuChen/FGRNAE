...
scNPF.pro <- function(x, network, gamma=0.5,qt.gene=0.4,qt.cell=0.5,nThreads=1) {
  ...
  # browser()
         ...
         if(is.character(network)){
          ...
          #2
          # idxr <- match(rownames(network),rownames(x))
          # idxc <- match(colnames(network),rownames(x))
          # network <- network[!is.na(idxr),!is.na(idxc)]
          # nullr <- Matrix::rowSums(network)==0
          # nullc <- Matrix::colSums(network)==0
          # network <- network[!nullr,!nullc]
         
          
          # #3 network的行列名与x的行名，取交集
          # idx=intersect(rownames(network),rownames(x))
          # idxx=intersect(colnames(network),idx)
          # network <- network[idxx,idxx]
          # 
          # nullrows <- Matrix::rowSums(network)==0
          # nullcols <- Matrix::colSums(network)==0
          # network <- network[!nullrows,!nullcols]
          # 
          # ind <- intersect(rownames(network),colnames(network))
          # network <- network[ind,ind]
